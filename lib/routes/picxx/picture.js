 ...
 * @LastEditors: Snow
 * @LastEditTime: 2022-01-13 21:02:37
 * @Description:
...
function getImages($info) {
    ...
    imgList.map(() =>
        imgs.push(
            `<p><img src="${$info('img').attr('src')}" data-pre-sourced="yes" data-sourced="yes" id="image2" data-original="${$info('img').attr('src')}" data-src="${$info('img').attr(
                'src'
            )}" "="" class="" style="max-width: 647px;"></p>`
        )
    );
    return imgs.join('');
}
